...
API_KEY = os.environ.get("GEMINI_API_KEY", "AQ.Ab8RN6IUJWjaetdGW9e58UxmkyRT0K7J0MPnpdbWi86-MDpNJg")
...
<head>
    <meta charset="UTF-8">
    <title>AI Twin Engine - مُحرك التوأم التاريخي الذكي</title>
...
<div class="container">
    <div class="header">
        <h1>
            <span data-ar-inline>🧠 مُحرك التوأم التاريخي الذكي</span>
            <span data-en-inline>🧠 AI Twin Engine</span>
        </h1>
        <p class="subtitle">
            <span data-ar-inline>مطابقة الأنماط الكمية والتوقع الاحتمالي</span>
            <span data-en-inline>Quantitative Pattern Matching & Probability Forecasting</span>
        </p>
    </div>
    ...
    <button class="btn" id="analyzeBtn" onclick="analyzeChart()">
        <span data-ar-inline>🔮 ابدأ هندسة التوقع المُركّب</span>
        <span data-en-inline>🔮 Start Composite Forecasting</span>
    </button>

    <div id="loading">
        <div class="spinner"></div>
        <div>
            <span data-ar-inline>جاري تحليل بصمة السيولة والتقاطع الزمني...</span>
            <span data-en-inline>Analyzing liquidity fingerprint & time confluence...</span>
        </div>
    </div>
    ...
    <div class="result-section" id="resultSection">
        ...
        <div class="section-box">
            <div class="section-title">
                <span data-ar-inline>🌳 شجرة القرارات الاحتمالية</span>
                <span data-en-inline>🌳 Probability Decision Tree</span>
            </div>
        ...
        <div class="section-box">
            <div class="section-title">
                <span data-ar-inline>📝 تحليل المحرك</span>
                <span data-en-inline>📝 Engine Analysis</span>
            </div>
...
<script>
    ...
    function renderTree(scenarios) {
        const tree = document.getElementById('treeVisual');
        const colors = ['#0ecb81', '#f0b90b', '#f6465d'];
        const titles = ['Bullish', 'Neutral', 'Bearish'];
        const titlesAr = ['صاعد', 'محايد', 'هابط'];

        tree.innerHTML = `
            <div class="tree-node-box" style="background: rgba(240,185,11,0.1); border: 2px solid #f0b90b;">
                <div class="title" style="color: #f0b90b;">${currentLang === 'ar' ? 'النقطة الحالية' : 'Current State'}</div>
                <div class="prob-val" style="color: #f0b90b;">100%</div>
            </div>
            <div class="tree-arrow">→</div>
            ${scenarios.map((s, i) => `
                <div class="tree-node-box" style="background: ${colors[i]}15; border: 2px solid ${colors[i]};">
                    <div class="title" style="color: ${colors[i]};">${currentLang === 'ar' ? titlesAr[i] : titles[i]}</div>
                    <div class="prob-val" style="color: ${colors[i]};">${s.probability}%</div>
                </div>
            `).join('<div class="tree-arrow">|</div>')}
        `;
    }
</script>
</body>
...
def generate_fallback_result():
    return {
        "features": [
            ...
            {"label": "Trend", "label_en": "Trend Strength", "value": "Medium"},
            {"label": "Volume", "label_en": "Volume Profile", "value": "High"},
            {"label": "Pattern", "label_en": "Candle Pattern", "value": "Bullish Engulfing"},
...
def predict():
    try:
        ...
        prompt = """Analyze this candlestick chart. You are an expert quantitative analyst.
          ...
          "scenarios": [
            {
              ...
              "risk_reward": "1:2.5",
              "candles": [
                {"open": 65000, "high": 65200, "low": 64800, "close": 65100}
              ]
            },
            {"probability": 35, "target": "±2%", "stop_loss": "±3%", "risk_reward": "1:1", "candles": []},
            {"probability": 20, "target": "-4%", "stop_loss": "+2%", "risk_reward": "1:2", "candles": []}
          ],
          "explanation": "Detailed technical analysis in Arabic with English terms in parentheses."
        }
